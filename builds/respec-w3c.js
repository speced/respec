...
// @ts-ignore
const hyperHTML = html$1;
/** @type {import("nanohtml/raw")} */
// @ts-ignore
const raw = rawBrowser;
/** @type {import("marked")} */
// @ts-ignore
...
function makeIssueSectionSummary(issueList) {
  ...
  issueList.hasChildNodes()
    ...
    : issueSummaryElement.append(hyperHTML`<p>${l10n$6.no_issues_in_spec}</p>`);
  if (
    !heading ||
...
function run$o() {
  ...
  if (tof.length && tofElement) {
    ...
    tofElement.append(
      hyperHTML`<h2>${l10n$7.list_of_figures}</h2>`,
...
function decorateFigure(figure, caption, i) {
  ...
  caption.prepend(l10n$7.fig, hyperHTML`<bdi class='figno'>${i + 1}</bdi>`, " ");
}
...
function createTableOfContents(ol) {
  ...
  const h2 = hyperHTML`<h2 class="introductory">${l10n$9.toc}</h2>`;
  addId(h2);
  nav.append(h2, ol);
  const ref =
...
function run$x() {
  Array.from(document.querySelectorAll("section.informative"))
    ...
    .forEach(heading => {
      heading.after(hyperHTML`<p><em>${l10n$a.informative}</em></p>`);
    });
}
