...
(function () {
  ...
  function run$13(config) {
    config.l10n = l10n$z[lang$1] || l10n$z.en;
  }

  var l10n$A = /*#__PURE__*/Object.freeze({
    __proto__: null,
    name: name$19,
    l10n: l10n$z,
    lang: lang$1,
...
>
  ...
  const modules = [
    ...
    Promise.resolve().then(function () { return l10n$A; }),
  ...
  const name$13 = "core/location-hash";
  ...
  async function getGroupDetails(group) {
    ...
    const hint =
      res.status === 404
        ...
        : undefined;
    showError(message, name$12, { hint });
  }
  ...
  const name$11 = "core/templates/show-link";
  ...
  const name$10 = "core/templates/show-logo";
  ...
  function showLogo(logo, index) {
    ...
    if (!logo.alt) {
      ...
      showError(msg, name$10, { hint, elements: [a] });
    }
    if (!logo.src) {
...
>
  ...
  function personToHTML(person) {
    ...
    >`;
      contents.push(html` - ${l10n$y.until(time)} `);
    }
    const dd = html`<dd
  ...
  var headersTmpl = (conf, options) => {
    ...
    <details open="${localStorage.getItem("tr-metadata") || "true"}">
      <summary>${l10n$x.more_details_about_this_doc}</summary>
      <dl>
        ${conf.thisVersion
          ? html`<dt>${l10n$x.this_version}</dt>
              <dd>
        ...
        ${"latestVersion" in conf // latestVersion can be falsy
          ? html`<dt>${l10n$x.latest_published_version}</dt>
              <dd>
                ${conf.latestVersion
        ...
        ${conf.edDraftURI
          ? html`
              <dt>${l10n$x.latest_editors_draft}</dt>
              <dd><a href="${conf.edDraftURI}">${conf.edDraftURI}</a></dd>
            `
          : ""}
        ${conf.historyURI || conf.github
          ? html`<dt>${l10n$x.history}</dt>
              ${conf.historyURI
                ? html`<dd>
        ...
        ${conf.testSuiteURI
          ? html`
              <dt>${l10n$x.test_suite}</dt>
              <dd><a href="${conf.testSuiteURI}">${conf.testSuiteURI}</a></dd>
            `
          : ""}
        ${conf.implementationReportURI
          ? html`
              <dt>${l10n$x.implementation_report}</dt>
              <dd>
        ...
        ${conf.prevED
          ? html`
              <dt>${l10n$x.prev_editor_draft}</dt>
              <dd><a href="${conf.prevED}">${conf.prevED}</a></dd>
            `
          : ""}
        ${conf.showPreviousVersion
          ? html`
              <dt>${l10n$x.prev_version}</dt>
        ...
        ${!conf.prevRecURI
          ...
          : conf.isRec
          ? html`
              <dt>${l10n$x.prev_recommendation}</dt>
              <dd><a href="${conf.prevRecURI}">${conf.prevRecURI}</a></dd>
            `
          : html`
              <dt>${l10n$x.latest_recommendation}</dt>
              <dd><a href="${conf.prevRecURI}">${conf.prevRecURI}</a></dd>
            `}
        ${conf.editors.length
          ? html`
              <dt>${conf.editors.length > 1 ? l10n$x.editors : l10n$x.editor}</dt>
          ...
          ? html`
              <dt>
                ${conf.formerEditors.length > 1
                  ? l10n$x.former_editors
                  : l10n$x.former_editor}
              </dt>
        ...
        ${conf.authors.length
          ? html`
              <dt>${conf.authors.length > 1 ? l10n$x.authors : l10n$x.author}</dt>
              ${showPeople(conf, "authors")}
            `
          : ""}
        ${conf.github || conf.wgPublicList
          ? html`<dt>${l10n$x.feedback}</dt>
              ${renderFeedback(conf)}`
          : ""}
        ${conf.errata
          ? html`<dt>Errata:</dt>
    ...
    ${conf.alternateFormats
      ? html`<p>
          ${l10n$x.multiple_alternates(options.multipleAlternates)}
  ...
  function renderFeedback(conf) {
    ...
    if (conf.wgPublicList) {
      ...
      const subject =
        conf.subjectPrefix ?? `[${conf.shortName}] ${l10n$x.your_topic_here}`;
    ...
    >`;
      ...
      const subjectLine =
        conf.subjectPrefix ||
        html`[${conf.shortName}] <em>${l10n$x.message_topic}</em>`;
      const emailSubject = html`${l10n$x.with_subject_line}${" "}
      <kbd>${subjectLine}</kbd>`;
  ...
  var sotdTmpl = (conf, opts) => {
    ...
    ${conf.isUnofficial
      ...
      : html`
          <p><em>${l10n$w.status_at_publication}</em></p>
  ...
  async function run$W(conf) {
    ...
    if ((conf.isCGBG || !conf.isNoTrack || conf.isTagFinding) && !sotd.id) {
      ...
      showError(msg, name$_, { elements: [sotd], hint });
    }
...
/>`);
    ...
    if (!conf.excludeGithubLinks) {
      const otherLink = {
        ...
        data: [
          ...
          {
            value: l10n$v.file_a_bug,
            href: newProps.issueBase,
          },
          {
            value: l10n$v.commit_history,
  ...
  async function run$Q() {
    ...
    const promisesToInclude = Array.from(includables).map(async el => {
      ...
      try {
        ...
        showError(msg, name$U, { elements: [el] });
      }
    });
  ...
  function run$P(conf) {
    ...
    if (h1Elem.isConnected && h1Elem.textContent.trim() === "") {
      ...
      showError(msg, name$T, { title, elements: [h1Elem] });
    }
  ...
  function setDocumentTitle(conf, h1Elem) {
    ...
    if (!h1Elem.isConnected) {
      h1Elem.textContent = document.title || `${l10n$u.default_title}`;
    }
  ...
  function run$O(conf) {
    ...
    if (!Number.isInteger(level) || level < 0) {
      ...
      showError(msg, name$S, { title, elements: [h1Elem] });
      return;
    }

    h1Elem.append(` ${l10n$t.level} ${level}`);
    document.title = `${document.title} ${l10n$t.level} ${level}`;
  ...
  async function run$N() {
    ...
    if (!abstract.querySelector("h2")) {
      abstract.prepend(html`<h2>${l10n$s.abstract}</h2>`);
    }
  }
  ...
  function findAbstract() {
    ...
    if (abstract) {
      switch (abstract.localName) {
        ...
        default:
          showError("The abstract should be a `<section>` element.", name$R, {
            elements: [abstract],
          });
          return abstract;
      }
    ...
    // using markdown.
    const searchString = l10n$s.abstract.toLowerCase();
  ...
  function run$K(conf) {
    ...
    if (!document.querySelector("section#references > :is(h2, h1)")) {
      ...
      refSection.prepend(html`<h1>${l10n$r.references}</h1>`);
    }

    refSection.classList.add("appendix");

    if (norms.length) {
      const sec = createReferencesSection(norms, l10n$r.norm_references);
      refSection.appendChild(sec);
    }
    if (informs.length) {
      const sec = createReferencesSection(informs, l10n$r.info_references);
      refSection.appendChild(sec);
    }
  ...
  function warnBadRefs(refs) {
    for (const { ref } of refs) {
      ...
      showError(msg, name$N, { hint, elements });
    }
  }
  ...
  function inlineBibrefMatches(matched, txt, conf) {
    ...
    if (illegal && !conf.normativeReferences.has(cleanRef)) {
      ...
      showWarning(msg, name$M, { elements: [citeElem], hint });
    }
  ...
  function run$J(conf) {
    ...
    const keywords = l10n$q.rfc2119Keywords();
  ...
  function processConformance(conformance, conf) {
    ...
    const content = html`
    <h1>${l10n$p.conformance}</h1>
    <p>${l10n$p.normativity}</p>
    ${terms.length ? l10n$p.keywordInterpretation(keywords, plural) : null}
  `;
  ...
  function computeExport(dfn) {
    switch (true) {
      ...
      case dfn.matches(".export.no-export"): {
        ...
        showError(msg, name$K, { elements: [dfn], hint });
        break;
      }
  ...
  function validateDefinition(text, type, dfn) {
    ...
    if (entry.requiresFor && !dfn.dataset.dfnFor) {
      ...
      showError(msg, name$K, { hint, elements: [dfn] });
    }

    if (entry.validator) {
      entry.validator(text, type, dfn, name$K);
    }
  }
...
`;
  ...
  function makeTitle(elem, num, report) {
    ...
    <a class="self-link">${l10n$o.example}<bdi>${number}</bdi></a
    >${title}
  </div>`;
  }
...
`;
  ...
  // @ts-check
  const name$H = "core/issues-notes";
        ...
        if (isIssue) {
          ...
          if (inno.dataset.hasOwnProperty("number")) {
            ...
            if (!ghIssue) {
              ...
              showWarning(msg, name$H);
            }
            if (ghIssue && !report.title) {
              report.title = ghIssue.title;
            }
          }
          issueList.append(createIssueSummaryEntry(l10n$n.issue, report, div.id));
        }
  ...
  function makeIssueSectionSummary(issueList) {
    ...
    issueList.hasChildNodes()
      ...
      : issueSummaryElement.append(html`<p>${l10n$n.no_issues_in_spec}</p>`);
    if (
      !heading ||
  ...
  const name$G = "core/best-practices";
  ...
  const lang = lang$1 in localizationStrings$m ? lang$1 : "en";
  ...
  function run$D() {
    ...
    [...bps].forEach((bp, num) => {
      ...
      ><bdi lang="${lang}">${l10n$m.best_practice}${num + 1}</bdi></a
    >`;
  ...
  function run$C() {
    ...
    if (tof.length && tofElement) {
      ...
      tofElement.append(
        html`<h1>${l10n$l.list_of_figures}</h1>`,
        html`<ul class="tof">
        ${tof}
      </ul>`
      );
  ...
  function collectFigures() {
    ...
    document.querySelectorAll("figure").forEach((fig, i) => {
      ...
      if (caption) {
        ...
        showWarning(msg, name$F, { elements: [fig] });
      }
    });
    return tof;
  }
  ...
  function decorateFigure(figure, caption, i) {
    ...
    caption.prepend(l10n$l.fig, html`<bdi class="figno">${i + 1}</bdi>`, " ");
  }
  ...
  function mapTitleToDfns() {
    ...
    for (const key of definitionMap.keys()) {
      ...
      if (duplicates.length > 0) {
        showError(l10n$k.duplicateMsg(key), name$B, {
          title: l10n$k.duplicateTitle,
          elements: duplicates,
        });
      }
    }
...
`;
  ...
  function renderLocalTerm(term, dfns) {
    ...
    <ul>
      ${dfns.map(dfn => {
        const type = getLocalTermType(dfn);
        const text = getLocalTermSuffix(dfn, type, term) || l10n$j.dfnOf;
   ...
   * that is preferred.
   */
  const name$w = "core/webidl-index";
  ...
  function skipFromToC() {
    ...
    for (const section of sections) {
      ...
      if (maxToc < 0 || maxToc > 6 || Number.isNaN(maxToc)) {
        ...
        showError(msg, name$v, { elements: [section] });
        continue;
      }
  ...
  function createTableOfContents(ol) {
    ...
    const h2 = html`<h2 class="introductory">${l10n$i.toc}</h2>`;
    addId(h2);
    nav.append(h2, ol);
    const ref =
  ...
  function run$s() {
    Array.from(document.querySelectorAll("section.informative"))
      ...
      .forEach(heading => {
        heading.after(html`<p><em>${l10n$h.informative}</em></p>`);
      });
  }
  ...
  function run$r(conf) {
    ...
    for (const h of headings) {
      ...
      const label = l10n$g.permalinkLabel(
...
`;
  ...
  function getEngineSupportIcons(engines) {
    if (engines.length === 3) {
      return html`<span title="${l10n$f.inAllEngines}">✅</span>`;
    }
    if (engines.length < 2) {
      return html`<span title="${l10n$f.inSomeEngines}">🚫</span>`;
    }
  ...
  function getEngineSupport(engines) {
    if (engines.length === 3) {
      return html`<p class="engines-all">${l10n$f.inAllEngines}</p>`;
    }
    if (engines.length < 2) {
      return html`<p class="engines-some">${l10n$f.inSomeEngines}</p>`;
    }
  }
  ...
  function run$o(conf) {
    const saveDialog = {
      async show(button) {
        ...
        ui.freshModal(l10n$e.save_snapshot, div, button);
      },
    };
    ...
    let button;
    if (supportsDownload) {
      button = ui.addCommand(l10n$e.save_snapshot, show, "Ctrl+Shift+Alt+S", "💾");
    }
  ...
  `;
    ui.freshModal(l10n$d.search_specref, specrefSearchUI, button$2);
  }
  ...
  const button$1 = ui.addCommand(l10n$c.title, show$1, "Ctrl+Shift+Alt+x", "📚");
  ...
  `;
    ui.freshModal(l10n$c.title, xrefSearchUI, button$1);
  }
  ...
  const button = ui.addCommand(
    `${l10n$b.about_respec} ${window.respecVersion}`,
    show,
    "Ctrl+Shift+Alt+A",
    "ℹ️"
  );
...
`;
    ui.freshModal(`${l10n$b.about_respec} - ${window.respecVersion}`, div, button);
  }
...
`;
  ...
  function run$k(conf) {
    ...
    if (!conf.testSuiteURI) {
      showError(l10n$a.missing_test_suite_uri, name$l);
      return;
    }
    ...
    return tests
      .map(test => {
        try {
          ...
          showWarning(msg, name$l, { elements: [elem] });
        }
      })
      .filter(href => href);
  }
  ...
  function handleDuplicates(testURLs, elem) {
    ...
    if (duplicates.length) {
      ...
      showWarning(msg, name$l, { hint, elements: [elem] });
    }
  }
    ...
    for (const elem of sortables) {
      ...
      switch (elem.localName) {
        ...
        default: {
          ...
          showWarning(msg, name$k, { elements: [elem] });
        }
      }
      if (sortedElems) {
...
`;
  ...
  const name$j = "core/highlight-vars";
...
`;
  ...
  const name$f = "core/anchor-expander";
  ...
  function run$e() {
    ...
    for (const a of anchors) {
      ...
      if (!matchingElement) {
        ...
        showError(msg, name$f, { title, elements: [a] });
        continue;
      }
    ...
    connectedCallback() {
      ...
      ${{
        any: fetchCommits(from, to, filter)
          ...
          .catch(error => showError(error.message, name$e, { elements: [this] }))
          .finally(() => {
  ...
  const name$d = "core/custom-elements/index";
  ...
  const name$c = "core/web-monetization";
  ...
  const ruleName$9 = "check-charset";
  const name$a = "core/linter-rules/check-charset";
  ...
  function run$a(conf) {
    ...
    showWarning(l10n$9.msg, name$a, { hint: l10n$9.hint, elements: [...metas] });
  }
  ...
  const ruleName$8 = "check-punctuation";
  const name$9 = "core/linter-rules/check-punctuation";
  ...
  function run$9(conf) {
    ...
    showWarning(l10n$8.msg, name$9, { hint: l10n$8.hint, elements: offendingElements });
  }
  ...
  const ruleName$7 = "check-internal-slots";
  const name$8 = "core/linter-rules/check-internal-slots";
  ...
  const ruleName$6 = "local-refs-exist";
  const name$7 = "core/linter-rules/local-refs-exist";
  ...
  const ruleName$5 = "no-unused-dfns";
  const name$6 = "core/linter-rules/no-unused-dfns";
  ...
  function run$6(conf) {
    if (!conf.lint?.[ruleName$5]) return;
    const logger = conf.lint[ruleName$5] === "error" ? showError : showWarning;
    /** @type NodeListOf<HTMLElement> */
    ...
    elements.forEach(element => {
      ...
      logger(l10n$5.msg(text), name$6, { elements, hint: l10n$5.hint });
    });
  }
  ...
  const ruleName$2 = "required-sections";
  const name$3 = "w3c/linter-rules/required-sections";
  ...
  const localizationStrings$2 = {
    en: {
      ...
      hint(sectionTitle) {
        ...
        or turn off the ${`[${ruleName$2}]`} linter rule.`;
      },
    ...
    es: {
      ...
      hint(sectionTitle) {
        ...
        o apaga la regla de linter ${`[${ruleName$2}]`}.`;
      },
  ...
  function run$3(conf) {
    ...
    if (!localizationStrings$2[resolveLanguageAlias(lang$1)]) {
      console.warn(`Missing localization strings for ${lang$1}.`);
      return;
    }
    ...
    const logger = conf.lint[ruleName$2] === "error" ? showError : showWarning;

    const missingRequiredSections = new InsensitiveStringSet([
      l10n$2.privacy_considerations,
      l10n$2.security_considerations,
    ]);
    ...
    for (const title of missingRequiredSections) {
      logger(l10n$2.msg(title), name$3, {
        hint: l10n$2.hint(title),
      });
    }
  }
