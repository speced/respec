  ...
  function(beautify, beautifyOpts, pubsubhub, utils, ui) {
    ...
    return {
      show: function() {
        if (!conf.diffTool)
          conf.diffTool =
            "https://www5.aptest.com/standards/htmldiff/htmldiff.pl";
        ...
        ui.freshModal("Bewaar Snapshot", $div, button);
      },
      ...
      toString: function() {
        ...
        var str = "<!DOCTYPE html", dt = doc.doctype;
        if (dt && dt.publicId)
      ...
      toXML: function(mode) {
        ...
        str += ">\n<html";
        var ats = doc.documentElement.attributes, hasxmlns = false;
        for (var i = 0; i < ats.length; i++) {
          var an = ats[i].name;
        ...
        var dumpNode = function(node) {
          ...
          if (
            ...
            out += noEsc[noEsc.length - 1]
              ? node.nodeValue
              : utils.xmlEscape(node.nodeValue);
          } else {
