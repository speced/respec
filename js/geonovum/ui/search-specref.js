...
define(["core/biblio", "core/ui"], function(biblio, ui) {
  ...
  function msg(query, count) {
    if (count) {
      return (
        "Er zijn " +
        pluralize(count, "resultaat", "resultaten") +
        " gevonden voor '" +
        query +
        "'."
      );
    ...
    return (
      "Voor '" +
      query +
      "' zijn geen resultaten gevonden in de Specref database.<br>Sorry. 😢"
    );
  }
  ...
  return {
    show: function() {
      var $halp = $(
        "<div><form><p><input name=q type=search> <input type=submit value=zoek /></p></form></div"
      );
