...
define(
    ...
    function (biblio, ui) {
        ...
        function msg(query, count) {
            if (count) {
                return "Er zijn " + pluralize(count, 'resultaat', 'resultaten') + " gevonden voor '" + query + "'.";
            }
            return "Voor '" + query + "' zijn geen resultaten gevonden in de Specref database.<br>Sorry. 😢";
        }
        ...
        const button = ui.addCommand("Zoek Specref DB", "geonovum/ui/search-specref", "Ctrl+Shift+Alt+space", "🔎");
        return {
            show: function() {
                var $halp = $("<div><form><p><input name=q type=search> <input type=submit value=zoek /></p></form></div");
                ...
                ui.freshModal("Zoek Specref DB", $halp, button);
                $search.focus();
