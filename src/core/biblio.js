...
function bibref(conf) {
  ...
  for (var item in badrefs) {
    ...
    if (badrefs.hasOwnProperty(item)) pubsubhub.pub("error", msg);
  }
}
...
export const biblio = {
  ...
  async updateFromNetwork(refs, options = {forceUpdate: false}) {
    ...
    await biblioDB.close();
    return data;
  },
  async resolveRef(key) {
    const biblio = await this.done;
  ...
  async run(conf, doc, cb) {
    ...
    if (externalRefs.length) {
      ...
      const data = await this.updateFromNetwork(externalRefs, {forceUpdate: true});
      Object.assign(conf.biblio, data);
    }
    Object.assign(conf.biblio, conf.localBiblio);
    bibref(conf);
    finish();
    await this.updateFromNetwork(neededRefs);
  }
};

export function resolveRef(ref){
  return biblio.resolveRef(ref);
}
...
export function stringifyRef(ref) {
  return stringifyReference(ref);
}
