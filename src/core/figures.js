define(["exports", "core/pubsubhub"], function (exports, _pubsubhub) {
  ...
  function run(conf, doc, cb) {
    ...
    $(".figure", doc).each(function (i, figure) {
      var $figure = $(figure),
          title = $figure.attr("title") || $figure.find("[title]").attr("title") || $figure.attr("alt") || $figure.find("[alt]").attr("alt") || "",
          $caption = $("<figcaption/>").text(title);

      // change old syntax to something HTML5 compatible
      if ($figure.is("div")) {
        (0, _pubsubhub.pub)("warn", "You are using the deprecated div.figure syntax; please switch to <figure>.");
        $figure.append($caption);
        $figure.renameElement("figure");
      } else {
        (0, _pubsubhub.pub)("warn", "You are using the deprecated img.figure syntax; please switch to <figure>.");
