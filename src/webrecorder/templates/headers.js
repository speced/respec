...
function linkLicense(licenseInfo) {
  ...
  return html`<a rel="license" href="${url}" title="${name}">${short}</a>`;
}
...
function renderOfficialCopyright(conf) {
  return html`<p class="copyright">
    ...
    &copy;
    ${conf.copyrightStart ? `${conf.copyrightStart}-` : ""}${conf.publishYear}
    <a href="https://webrecorder.net/"><abbr title="Webrecorder">Webrecorder</abbr></a> ${linkLicense(conf.licenseInfo)}
  </p>`;
}
