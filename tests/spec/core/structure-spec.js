describe("Core - Structure", function () {
    ...
    it("should build a ToC with default values", function () {
        ...
        runs(function () {
            ...
            expect($toc.find("ul:first").attr("id")).toEqual("respecContents");
            expect($toc.find("ul:first").attr("role")).toEqual("directory");
            expect($toc.find("h2").text()).toEqual("Table of Contents");
            flushIframes();
        });
    ...
    it("should enclose content sections with appropriate roles", function() {
        var doc;
        runs(function () {
            makeRSDoc({ config: basicConfig, body: body }, function (rsdoc) { doc = rsdoc; });
        });
        waitsFor(function () { return doc; }, MAXOUT);
        runs(function () {
            var $main = $("#respecMain", doc)
            ,   $app  = $("#respecAppendices", doc)
            ;
            expect($main.attr('role')).toEqual("main");
            expect($app.attr('role')).toEqual("complementary");
            flushIframes();
        });
    });
